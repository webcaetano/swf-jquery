/**
 * v1.2.0
 * Copyright (c) 2015  Authors Andre Caetano, Jonathan Neal
 * swfJQ.js is open sourced under the MIT license.
 */
(function($, flash, Plugin) {
	...
	function _objectToArguments(obj) {
		...
		for(var i in obj) arr.push((typeof obj[i] == OBJECT ? _objectToArguments(obj[i]) : [i, (ENCODE) ? encodeURI(obj[i]) : obj[i]].join('=')));
	...
	function _objectFromObject(obj) {
		...
		for(var i in obj) if(obj[i]) arr.push([i, '="', obj[i], '"'].join(''));
	...
	function _paramsFromObject(obj) {
		...
		for(var i in obj) arr.push(['<param name="', i,'" value="', _objectToArguments(obj[i]), '" />'].join(''));
		return arr.join('');
	}
	...
	$[flash] = {
		...
		create: function (obj) {
			...
			if(!obj.swf || instance.expressInstallIsActive || (!instance.available && !obj.hasVersionFail)) return false;

			if(!instance.hasVersion(obj.hasVersion || 1)) {
				instance.expressInstallIsActive = true;

				if(typeof obj.hasVersionFail == 'function' && !obj.hasVersionFail.apply(obj)) return false;
			...
			var attrs = {
				...
				id: obj.id || 'flash_' + (+new Date).toString() + Math.floor(Math.random() * 999999999),
				width: obj.width || 320,
			...
			var toDel = ['fallback','hasVersion','hasVersionFail','height','id','swf','useEncode','width'];
			for(var i=0;i<toDel.length;++i) delete obj[toDel[i]];
